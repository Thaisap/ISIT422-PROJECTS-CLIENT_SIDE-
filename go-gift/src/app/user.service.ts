...
CreateProfile(body:Profile) : Observable<Profile> {
  return this.http.post<Profile> ('https://localhost:3000/profile', body, this.httpOptions);
}

submitRegister(body: any){
  return this.http.post('https://localhost:3000/usercredential/register', body,{
    observe:'body'
  });
}
/* 
...
postSocialLogin( socialData: any): Observable<any> {
  return this.http.post('https://localhost:3000/usercredential/postSocialLogin', socialData)
}
...
login(body: any): Observable<Credentials>{
  return this.http.post<Credentials>('https://localhost:3000/usercredential/login', body)
//  ,{  observe:'body'});
}
...
credentials (body: any, id: string): Observable<Object>{
  return this.http.patch<Object>(`https://localhost:3000/usercredential/credentials/${id}`, body)
//  ,{  observe:'body'});
}

getUserName() {
  return this.http.get('https://localhost:3000/usercredential/username', {
    observe: 'body',
...
getAllTags() : Observable<allTags> {
  return this.http.get<allTags> ('https://localhost:3000/allTags');
}

//Create Account Page: used to create a new tag doc in the tag collection
CreateTag(body:WriteTagDoc) : Observable<string> {
  return this.http.post<string> ('https://localhost:3000/tag', body, this.httpOptions);
}

// Profile Page: used to populate data
getCurrentUser(id: string): Observable<Profile>{
  return this.http.get<Profile>(`https://localhost:3000/profile/${id}`);
}
...
sendFriendEmail(body: EmailDoc): Observable<string>{
  return this.http.post<string>(`https://localhost:3000/friend/email`, body, this.httpOptions);
}
...
takeWishlist(id: string):Observable<Profile>{
  return this.http.get<Profile>(`https://localhost:3000/takeWishlist/${id}`);
}
...
getItemListByTagName(tagName: string): Observable<Item[]>{
  return this.http.get<Item[]>(`https://localhost:3000/itemsByTag/${tagName}`);
}

//Create Wishlist Item Page: used to create a new item in the item collection
createItem(body: WriteItemDoc) : Observable<string> {
  return this.http.post<string> ('https://localhost:3000/item', body, this.httpOptions);
}
...
addItemToTag(tagId: string, itemId: string): Observable<tag>{
  return this.http.patch<tag>(`https://localhost:3000/tag/${tagId}`, [itemId], this.httpOptions)
}
...
createUserWithImg(body: FormData): Observable<ProfileWithImg>{
  return this.http.post<ProfileWithImg>('https://localhost:3000/profileWithImg', body)
}

getUserWithImg(userId: string): Observable<ProfileWithImg>{
  return this.http.get<ProfileWithImg>(`https://localhost:3000/profileWithImg/${userId}`);
}

updatePersonalInfo(userId: string, body: Object): Observable<ProfileWithImg>{
  console.log(body);
  return this.http.patch<ProfileWithImg>(`https://localhost:3000/profileWithImg/info/${userId}`, body, this.httpOptions);
}

updateBio(userId: string, body: Object): Observable<ProfileWithImg>{
  console.log(body);
  return this.http.patch<ProfileWithImg>(`https://localhost:3000/profileWithImg/bio/${userId}`, body, this.httpOptions);
}

updateProfilePicture(userId: string, body: FormData): Observable<ProfileWithImg>{
  return this.http.patch<ProfileWithImg>(`https://localhost:3000/profileWithImg/image/${userId}`, body);
}

// [NEW] update tag in tag array for profile with image
updateTagInUser(userId: string, tagIds: string[]): Observable<ProfileWithImg>{
  return this.http.patch<ProfileWithImg>(`https://localhost:3000/profileWithImg/tag/${userId}`, tagIds, this.httpOptions);
}
...
getWishlistForUserWithImg(userId: string): Observable<ProfileWithImg>{
  return this.http.get<ProfileWithImg>(`https://localhost:3000/profileWithImg/wishlist/${userId}`);
}
...
getUserInfo() : Observable<User> {
  return this.http.get<User> ('https://localhost:3000/getUserInfo');
}

getFriendInfo() : Observable<User[]> {
  return this.http.get<User[]> ('https://localhost:3000/getFriendInfo');
}
